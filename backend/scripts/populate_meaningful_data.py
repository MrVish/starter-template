...
This will create sensible relationships between fact and dimension tables.

Run from the backend directory with:
python scripts/populate_meaningful_data.py
"""
...
def populate_fact_transactions(conn, dim_data):
    ...
    if transactions_count > 0:
        print(f"fact_transactions already has {transactions_count} rows. Skipping.")
    else:
        ...
        for i in range(NUM_TRANSACTIONS):
            try:
                ...
                campaign_id = random.choice(dim_data['campaign_ids']) if random.random() < 0.3 else None
                channel_id = random.choice(dim_data['channel_ids']) if random.random() < 0.3 else None
                
                # For fact_transactions
                cursor.execute('''
                INSERT INTO fact_transactions (
                    customer_id, txn_date, product_code, product_key, txn_amount, 
    ...
    # Check if datavault table has data already
    cursor.execute("SELECT COUNT(*) FROM fact_transactions_datavault")
    datavault_count = cursor.fetchone()[0]
    if datavault_count > 0:
        print(f"fact_transactions_datavault already has {datavault_count} rows. Skipping.")
    else:
        # Generate datavault transaction data
        transaction_types = ["PURCHASE", "PAYMENT", "WITHDRAWAL", "DEPOSIT", "TRANSFER", "FEE"]
        currencies = ["USD", "EUR", "GBP", "CAD"]
        ...
        for i in range(NUM_TRANSACTIONS):
            try:
                ...
                product_key = random.choice(dim_data['product_keys'])
                branch_key = random.choice(dim_data['branch_keys']) if random.random() < 0.7 else None
                txn_type = random.choice(transaction_types)
                txn_amount = random.uniform(5, 5000)
                ...
                ''', (
                    ...
                    random.choice(dim_data['merchant_categories']),
                    txn_date.isoformat(),
...
def populate_fact_spend_cc(conn, dim_data):
    ...
    if existing_count > 0:
        print(f"fact_spend_cc already has {existing_count} rows. Skipping.")
        return
    ...
    for i in range(NUM_CREDIT_CARD_TXNS):
        try:
            ...
            if i % 50 == 0:
                conn.commit()
                print(f"Inserted {i+1} transactions into fact_spend_cc")
        except Exception as e:
...
def populate_fact_repayments(conn, dim_data):
    ...
    existing_count = cursor.fetchone()[0]
    if existing_count > 0:
        print(f"fact_repayments already has {existing_count} rows. Skipping.")
        return
    ...
    for i in range(NUM_REPAYMENTS):
        try:
            ...
            if i % 50 == 0:
                conn.commit()
                print(f"Inserted {i+1} repayments into fact_repayments")
        except Exception as e:
...
def populate_fact_customer_channel_activity(conn, dim_data):
    ...
    existing_count = cursor.fetchone()[0]
    if existing_count > 0:
        print(f"fact_customer_channel_activity already has {existing_count} rows. Skipping.")
        return
    ...
    for i in range(NUM_CHANNEL_ACTIVITIES):
        try:
            ...
            if i % 50 == 0:
                conn.commit()
                print(f"Inserted {i+1} activities into fact_customer_channel_activity")
        except Exception as e:
...
def populate_fact_campaign_performance(conn, dim_data):
    ...
    existing_count = cursor.fetchone()[0]
    if existing_count > 0:
        print(f"fact_campaign_performance already has {existing_count} rows. Skipping.")
        return
    ...
    print(f"fact_campaign_performance table populated with {performance_id-1} records!")
...
def populate_fact_segment_performance(conn, dim_data):
    ...
    existing_count = cursor.fetchone()[0]
    if existing_count > 0:
        print(f"fact_segment_performance already has {existing_count} rows. Skipping.")
        return
    ...
    conn.commit()
    print(f"fact_segment_performance table populated with {performance_id-1} records!")
...
def populate_fact_channel_performance(conn, dim_data):
    ...
    existing_count = cursor.fetchone()[0]
    if existing_count > 0:
        print(f"fact_channel_performance already has {existing_count} rows. Skipping.")
        return
    ...
    conn.commit()
    print(f"fact_channel_performance table populated with {performance_id-1} records!")
...
def populate_fact_feedback(conn, dim_data):
    ...
    existing_count = cursor.fetchone()[0]
    if existing_count > 0:
        print(f"fact_feedback already has {existing_count} rows. Skipping.")
        return
